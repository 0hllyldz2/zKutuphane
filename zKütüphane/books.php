...
<body>
    ...
    <main>
        <div class="container mt-5">
                <div class="card"> <br>
                    <h1 class="signup-title">Kitaplar</h1>
                    <form action="operation.php" method="post" class="signup-form">
                        <input type="hidden" name="islem" value="kitaplar"> <br>
                        <?php
                            $kitaplar = $db->query('SELECT kitaplar.id, kitaplar.kitap_ad, kitaplar.yazar_adi, kitaplar.yayinevi, turler.tur_adi, kitaplar.stok, kitaplar.durum
                            FROM kitaplar
                            INNER JOIN turler
                            ON kitaplar.tur_id = turler.id ORDER BY kitaplar.id ASC')->fetchAll(PDO::FETCH_ASSOC);
                        ?>
                        <table class="table table-bordered table-dark table-hover">
                            ...
                            <?php
                                // Kitap Silme - Güncelleme İşlemi 
                                foreach ($kitaplar as $kitap) { ?>
                                    <tr>
                                        <td class="text-center"><a onClick="sil(<?=$kitap['id']; ?>)" class="btn btn-danger">Sil</a></td>
                                        <td><?=$kitap['id']; ?></td>
                                        <td><?=$kitap['kitap_ad']; ?></td>
                                        <td><?=$kitap['yazar_adi']; ?></td>
                                        ...
                                        <td class="text-center">
                                            <?php if ($kitap['stok'] > 0) { ?>
                                                <a href="giving_books.php?kitap_id=<?=$kitap['id']; ?>" class="btn btn-dark">Kitap Ver</a>
                                             <?php }
                                                else {
                                                    echo 'Kitabın teslim edilmesi bekleniyor!';
                                                } ?>  
                                        </td>  
                                        <td class="text-center"><a href="update_book.php?id=<?=$kitap['id']; ?>" class="btn btn-info">Güncelle</a></td>
                                    </tr>
                            <?php } ?>
                    ...
                    function sil(id) {
                        ...
                        cancelButtonText: "Hayır, İptal Et."
                            ...
                            if (result.isConfirmed) {
                                window.location.href = 'operation.php?islem=kitap_sil&id=' + id;
                            }
                        });
                    }
                </script>
