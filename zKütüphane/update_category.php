...
<body>
    ...
    <?php if (isset($_GET['id'])) {
            $id = $_GET['id'];
            $kitap_turu = $db->query("SELECT * FROM turler WHERE id=$id")->fetch(PDO::FETCH_ASSOC);
        }
         ?>

    <div class="container mt-5">
            <div class="card"> <br>
                <h1 class="signup-title">Kitap Türü Güncelle</h1>
                <form action="operation.php" method="post" class="signup-form">
                    <input type="hidden" name="islem" value="tur_guncelle">
                    <input type="hidden" name="id" value="<?=$kitap_turu['id'];?>">

                    <Label>Türler</Label>
                    <input type="text" class="form-control" name="tur_adi" value="<?=$kitap_turu['tur_adi'];?>" required>

                    <Label>Açıklaması</Label>
                    <input class="form-control" name="tur_aciklama" value="<?=$kitap_turu['tur_aciklama'];?>" required>

                    <Label>Kitap Durumu</Label>
                    <input type="checkbox" class="form-check-input" name="durum" <?php echo ($kitap_turu['durum'] == 1) ? 'checked' : ''; ?>><br><br>

                    <button class="btn btn-success">Kitap Türü Güncelle</button> <br><br>
                </form>
            </div>  
        </div>
        <div><br><br></div>
