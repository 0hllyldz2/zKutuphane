...
<body>
    ...
    <main>
        <div class="container mt-5">
                <div class="card"> <br>
                    <h1 class="signup-title">Kullanıcılar</h1>
                    <form action="operation.php" method="post" class="signup-form">
                        <input type="hidden" name="islem" value="kitaplar"> <br>
                        <?php
                            $kullanicilar = $db->query('SELECT kullanicilar.id, kullanicilar.ad_soyad, yetkiler.ad, kullanicilar.durum
                            FROM kullanicilar
                            INNER JOIN yetkiler
                            ON kullanicilar.yetki_id = yetkiler.id ORDER BY kullanicilar.id ASC')->fetchAll(PDO::FETCH_ASSOC);
                        ?>
                        <table class="table table-bordered table-dark table-hover">
                            ...
                            foreach ($kullanicilar as $kullanici) { ?>
                                <tr>
                                    <td class="text-center"><a onclick="sil(<?=$kullanici['id']; ?>)" class="btn btn-danger">Sil</a></td>
                                    <td><?=$kullanici['id']; ?></td>
                                    <td><?=$kullanici['ad_soyad']; ?></td>
                                    <td><?=$kullanici['ad']; ?></td>
                                    <td><?=$kullanici['durum']; ?></td>
                                    <td class="text-center"><a href="users_update.php?id=<?=$kullanici['id']; ?>" class="btn btn-danger">Güncelle</a></td>
                                </tr> <?php 
                                } ?>
                        </table>
                    </form>
                    ...
                    function sil(id) {
                        ...
                        cancelButtonText: "Hayır, İptal Et."
                            ...
                            if (result.isConfirmed) {
                                window.location.href = 'operation.php?islem=users_sil&id=' + id;
                            }
                        });
                    }
                </script>
